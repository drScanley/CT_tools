...
ColumnNames = ['file_name','X_voxel_size_mm','Y_voxel_size_mm','Z_voxel_size_mm','voltage_kv','amperage_ua','watts','exposure_time','filter','projections','frame_averaging','path_ID']
...
for filename in FileNames:
	...
	PathID = os.path.realpath(filename)
	FileID = re.search('([^\/]*)\.pca',filename).group(1) # pull out file name
	RowEntry = [FileID, VoxelSize, VoxelSize, VoxelSize, Voltage, Current, Watts, ExposureTime, Filter, NumberImages, Avg,PathID]
	Results[i] = RowEntry
	i = i+1
	# print(RowEntry)
